...
public class DemoScript : MonoBehaviour {
	...
	public string filesPath = @"%USERPROFILE%\Desktop";
	...
	void Start() {
		...
		RainityFile[] files = Rainity.GetFiles(Environment.ExpandEnvironmentVariables(filesPath));
	...
	void Update() {
		Vector2 defaultMousePos = Mouse.current != null ? Mouse.current.position.ReadValue() : Vector2.zero;
		inputStats.text = "Rainity Input - X:" + RainityInput.mousePosition.x + ", Y:" + RainityInput.mousePosition.y + "\nDefault Input - X:" + defaultMousePos.x + ", Y: " + defaultMousePos.y;
	}
