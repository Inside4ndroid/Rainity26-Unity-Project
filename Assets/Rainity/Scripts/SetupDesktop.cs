...
public class SetupDesktop : MonoBehaviour {
	...
	public static void Initialize () {
		...
		if (behindIcons) {
			...
			// Fallback 1: WorkerW may be a direct child of Progman on some Windows versions
			if (workerW == IntPtr.Zero) {
				workerW = FindWindowEx(progman, IntPtr.Zero, "WorkerW", IntPtr.Zero);
			}

			// Fallback 2: use Progman itself so at minimum we end up below the icon layer
			if (workerW == IntPtr.Zero) {
				workerW = progman;
				UnityEngine.Debug.LogWarning("Rainity: Could not find dedicated WorkerW for Behind Icons — using Progman as fallback parent.");
			}
		}
		...
		if (!UnityEngine.Application.isEditor) {
			...
			if (behindIcons && workerW != IntPtr.Zero) {
				WinAPI.SetWindowPos(handle, HWND_BOTTOM, (int)windowOffset.x, (int)windowOffset.y, fWidth, fHeight, 0);
				SetParent(handle, workerW);
				WinAPI.SetWindowPos(handle, HWND_BOTTOM, (int)windowOffset.x, (int)windowOffset.y, fWidth, fHeight, 0);
			} else if (behindIcons) {
				UnityEngine.Debug.LogError("Rainity: Behind Icons failed — no valid parent window found.");
			} else if (neverHide) {
